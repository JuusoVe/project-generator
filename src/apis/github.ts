...
const createRepoSecret = async (username: string, repoName: string, secretName: string, secretValue: string, keyId: string) => await githubAPIClient.rest.actions.createOrUpdateRepoSecret({ owner: username, repo: repoName, secret_name: secretName, encrypted_value: secretValue, key_id: keyId })
